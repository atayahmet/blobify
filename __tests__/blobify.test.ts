...
it('Blobify class tests:pipe method', async () => {
  const pipeline = pipe(() => 'Hello');
  expect (pipeline).toBeInstanceOf(Blobify);

  pipeline.pipe((str: string) => str + ' World')
    .pipe((str: string) => str + '!');

  expect(await pipeline.run()).toEqual('Hello World!');
});
...
it('base64Encode test', async () => {
  ...
  const isMatched = /data:.*;base64/.test(base64);
  expect(isMatched).toEqual(true);
  expect(atob(removeBase64Data(base64)).length).toEqual(text.length);
});
